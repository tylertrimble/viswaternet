...
def draw_nodes(model,ax,node_list,parameter_results=None,vmin=None,vmax=None,node_size=None,node_color='k',cmap='tab10',node_shape='.',edge_colors='k',line_widths=0,label=None):
    ...
    if len(parameter_results) != 0:
        for value in parameter_results:
            ...
            if value < -1e-5:
                ...
                if vmin is None and vmax is None:
                    g = nxp.draw_networkx_nodes(model['G'], model['pos_dict'], ax=ax, nodelist=node_list,node_size = node_size, node_color=parameter_results,vmax=np.max(parameter_results),vmin=-np.max(parameter_results), cmap=cmap,node_shape = node_shape,linewidths=line_widths,edgecolors=edge_colors,label=label)
                else:
                    g = nxp.draw_networkx_nodes(model['G'], model['pos_dict'], ax=ax, nodelist=node_list,node_size = node_size, node_color=parameter_results,vmax=vmax,vmin=vmin, cmap=cmap,node_shape = node_shape,linewidths=line_widths,edgecolors=edge_colors,label=label)
        ...
        if negativeValues:
            ...
            if vmin is None and vmax is None:
                g = nxp.draw_networkx_nodes(model['G'], model['pos_dict'], ax=ax, nodelist=node_list,node_size = node_size, node_color=parameter_results, cmap=cmap, node_shape = node_shape,linewidths=line_widths,edgecolors=edge_colors)
            else:
                g = nxp.draw_networkx_nodes(model['G'], model['pos_dict'], ax=ax, nodelist=node_list,node_size = node_size, node_color=parameter_results, cmap=cmap, node_shape = node_shape,linewidths=line_widths,edgecolors=edge_colors, vmin=vmin, vmax=vmax)
        ...
        nxp.draw_networkx_nodes(model['G'], model['pos_dict'], ax=ax, nodelist=node_list,node_size = node_size, node_color=node_color, node_shape = node_shape,edgecolors=edge_colors,linewidths=line_widths,label=label)
...
def plot_continuous_nodes(model,ax,parameter=None, value=None, unit=None,vmin=None,vmax=None,get_tanks=False,get_reservoirs=False,tanks=True, reservoirs=True, pumps=True, valves=True,cmap= default_cmap, color_bar_title=None,node_size=100, node_shape='.',edge_colors=None,line_widths=None,legend=True,legend_loc='upper right',savefig=True, save_name=None):
    ...
    if parameter is not None:
        ...
        parameter_results, node_list = processing.get_parameter(model,'node',parameter, value=value, tanks=get_tanks, reservoirs=get_reservoirs)
        
        if unit is not None:
            parameter_results = unit_conversion(parameter_results,parameter,unit)
            
        g = draw_nodes(model,ax,node_list,parameter_results=parameter_results,vmin=vmin,vmax=vmax,node_size=node_size,cmap=cmap,node_shape=node_shape,edge_colors=edge_colors,line_widths=line_widths)
    ...
    if legend:
        ...
        base.draw_legend(ax,pumps=pumps,loc=legend_loc)
...
def plot_continuous_links(model,ax,parameter=None,value=None,unit=None,min_width=1,max_width=5,vmin=None,vmax=None,tanks=True, reservoirs=True, pumps=True, valves=True,cmap=default_cmap,color_bar_title=None,legend=True,legend_loc='upper right',legend_title=None,savefig=True, save_name=None):
    ...
    if parameter is not None:
        ...
        parameter_results, link_list = processing.get_parameter(model,'link',parameter, value=value)
        ...
        g = draw_links(model,ax,link_list,parameter_results=parameter_results,cmap=cmap,widths=widths,vmin=vmin,vmax=vmax)
    ...
    if legend:
        ...
        base.draw_legend(ax,title=legend_title,pumps=pumps,loc=legend_loc)
