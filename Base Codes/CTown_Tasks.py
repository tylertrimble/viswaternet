...
# %%
def patternMatch(nodePattern,pattern, junc_name):
    global demandPatternNodes
...
for i in range(len(junc_names)):
    for pattern in patterns:
        patternMatch(demandPatterns[i],pattern,junc_names[i])
cmapValue = 1/6
for pattern in patterns:
...
legend1 = plt.legend(title='Demands (GPM)',labels=patterns, loc='lower right',fontsize = '15', title_fontsize = '17')
legend2 = plt.legend(title='Node Types', handles=handles, loc='upper right',fontsize = '15', title_fontsize = '17')
ax.add_artist(legend1)
ax.add_artist(legend2)

plt.savefig(image_path+'/DemandPatterns.png') 